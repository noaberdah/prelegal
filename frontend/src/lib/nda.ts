...
export type MNDATermKind = "expires" | "until_terminated";
...
export const mndaTermText = (data: NDAFormData): string =>
  data.mndaTermKind === "expires"
    ? `Expires ${data.mndaTermYears} ${
        data.mndaTermYears === 1 ? "year" : "years"
      } from the Effective Date.`
    : "Continues until terminated in accordance with the terms of the MNDA.";

export const confidentialityTermText = (data: NDAFormData): string =>
  data.confidentialityTermKind === "years"
...
// Segments allow the renderers (HTML + PDF) to style defined-term references
// (e.g., "Purpose", "Effective Date") consistently as underlined inline tokens
// that point back to the Cover Page.
export type Segment = { kind: "text" | "ref"; value: string };

const seg = (value: string): Segment => ({ kind: "text", value });
const ref = (value: string): Segment => ({ kind: "ref", value });
...
export const STANDARD_TERMS: StandardTermSection[] = [
  ...
  },
  {
    ...
    body: [
      seg("The Receiving Party shall: (a) use Confidential Information solely for the "),
      ref("Purpose"),
      seg(
  ...
  },
  {
    ...
    body: [
      seg("This MNDA commences on the "),
      ref("Effective Date"),
      seg(" and expires at the end of the "),
      ref("MNDA Term"),
      seg(
      ...
      ref("Term of Confidentiality"),
      seg(", despite any expiration or termination of this MNDA."),
    ],
  },
  {
    number: 6,
  ...
  },
  {
    ...
    body: [
      ...
      ref("Governing Law"),
      seg(", without regard to the conflict of laws provisions of such "),
      ref("Governing Law"),
      seg(
        ". Any legal suit, action, or proceeding relating to this MNDA must be instituted in the federal or state courts located in "
      ),
      ref("Jurisdiction"),
      seg(". Each party irrevocably submits to the exclusive jurisdiction of such "),
      ref("Jurisdiction"),
      seg(" in any such suit, action, or proceeding."),
    ],
  },
  {
    number: 10,
